...
func (g Galleries) Show(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		http.Error(w, "error with loading images", http.StatusInternalServerError)
		return
	}
...
func (g Galleries) UploadImage(w http.ResponseWriter, r *http.Request) {
	...
	for _, fileHeader := range fileHeaders {
		...
		defer file.Close()
		err = g.GalleryService.UploadImage(gallery.ID, fileHeader.Filename, file)
		if err != nil {
			var fileErr models.FileError
			if errors.As(err, &fileErr) {
				msg := fmt.Sprintf(
					"%v has an invalid content type or extention. Only png, jpeg and gif are allowed,",
					fileHeader.Filename,
				)
